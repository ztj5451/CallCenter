...
public class ServerFinishActivity extends Activity {
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case HandlerException.Success:
				Toast.makeText(ServerFinishActivity.this, "审批成功!",
						Toast.LENGTH_SHORT).show();
				clearData();
				ServerFinishActivity.this.finish();
				overridePendingTransition(R.anim.in_to_right,
						R.anim.out_from_left);
				break;
			case HandlerException.Fail:
				Toast.makeText(ServerFinishActivity.this, "审批失败!",
						Toast.LENGTH_SHORT).show();
				break;
			...
			case HandlerException.AllSuccess:
				...
				adapter = new ArrayAdapter<String>(ServerFinishActivity.this,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.server_finish);
		init();
		Event();
		new Thread() {
			public void run() {
	...
	private void init() {
		settingUtils = new SettingUtils(ServerFinishActivity.this);
	...
	private void clearData() {
		tdbzEdit.setText("");
		tdbzEdit.setHint("请输入审批备注!");
	}

	private boolean checkData() {
		tdbz = tdbzEdit.getText().toString().trim();
		if (tdbz.length() == 0) {
			Toast.makeText(ServerFinishActivity.this, "请输入审批备注!",
					Toast.LENGTH_SHORT).show();
			return false;
		}
		return true;
	...
	private StringBuffer gwtdUrl() {
		...
		try {
			gwtdUrl.append(SettingUtils.get("gwsp_sp_url")).append("id=")
					.append(id).append("&gwid=").append(gwId)
					.append("&loginname=").append(loginname).append("&myd=")
					.append(mydId).append("&tdbeizhu=")
					.append(UrlEncode.Encode2(tdbz));
		} catch (Exception e) {
			// TODO: handle exception
		}
		System.out.println("公文审批url:" + gwtdUrl);
		return gwtdUrl;
	}
